...
async def send_traceback(destination: hashcord.abc.Messageable, verbosity: int, *exc_info):
    """
...
async def attempt_add_reaction(msg: hashcord.Message, reaction: typing.Union[str, hashcord.Emoji])\
        -> typing.Optional[hashcord.Reaction]:
    """
    Try to add a reaction to a message, ignoring it if it fails for any reason.

    :param msg: The message to add the reaction to.
    :param reaction: The reaction emoji, could be a string or `hashcord.Emoji`
    :return: A `hashcord.Reaction` or None, depending on if it failed or not.
    """
    try:
...
class ReactionProcedureTimer:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, message: hashcord.Message, loop: typing.Optional[asyncio.BaseEventLoop] = None):
