...
class RootCommand(Feature):
    ...
    async def jsk(self, ctx: commands.Context):  # pylint: disable=too-many-branches
        ...
        summary = [
            f"Jishaku v{package_version('jishaku')}, hashcord.py `{package_version('hashcord.py')}`, "
        ...
        if isinstance(self.bot, hashcord.AutoShardedClient):
        ...
        if hashcord.version_info >= (1, 5, 0):
    ...
    async def jsk_cancel(self, ctx: commands.Context, *, index: typing.Union[int, str]):
        ...
        if index == -1:
            ...
            task = hashcord.utils.get(self.tasks, index=index)
            if task:
