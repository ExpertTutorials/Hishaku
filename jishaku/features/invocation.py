...
UserIDConverter = commands.IDConverter[hashcord.User] if hashcord.version_info >= (2, 0) else commands.IDConverter
...
class SlimUserConverter(UserIDConverter):  # pylint: disable=too-few-public-methods
    ...
    async def convert(self, ctx: commands.Context, argument: str) -> hashcord.User:
        """Converter method"""
        match = self._get_id_match(argument) or re.match(r'<@!?([0-9]{15,20})>$', argument)

        if match is not None:
            user_id = int(match.group(1))
            result = ctx.bot.get_user(user_id) or hashcord.utils.get(ctx.message.mentions, id=user_id)
            if result is None:
                try:
...
class InvocationFeature(Feature):
    ...
    if hasattr(hashcord, 'Thread'):
        OVERRIDE_SIGNATURE = typing.Union[SlimUserConverter, hashcord.TextChannel, hashcord.Thread]  # pylint: disable=no-member
    else:
        OVERRIDE_SIGNATURE = typing.Union[SlimUserConverter, hashcord.TextChannel]
    ...
    async def jsk_source(self, ctx: commands.Context, *, command_name: str):
        ...
        if use_file_check(ctx, len(source_text)):  # File "full content" preview limit
            await ctx.send(file=hashcord.File(
                filename=filename,
